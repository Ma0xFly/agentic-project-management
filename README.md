...
- 不保留英文原版契约层，避免占用上下文并诱导 Agent 输出英文项目文档。
- 用中文吸收上游 APM 的关键流程约束：Planner/Manager/Worker 职责边界、审批门槛、上下文边界、依赖判定、日志格式、Message Bus、Handoff、Recovery、Tracker 和 Memory。
- 面向用户的对话、`.apm/spec.md`、`.apm/plan.md`、`Rules`、Task Prompt、Task Log、Task Report、Handoff Log、Stage Summary、Memory Notes 等正文都必须使用中文。

这样做会让 release zip 比压缩版更大，但比双语版更节省上下文。APM 的“性能”主要来自高密度流程约束，而不是文件体积越小越好。后续维护时不要为了减小 zip 体积删除关键流程约束。
