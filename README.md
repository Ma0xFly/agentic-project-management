...
- **中文执行层**：本分叉的实际执行口径和输出语言权威。面向用户的对话、`.apm/spec.md`、`.apm/plan.md`、`Rules`、Task Prompt、Task Log、Task Report、Handoff Log、Stage Summary、Memory Notes 等正文都必须使用中文。
- **原版契约层**：仅作为流程完整性兜底，维持 Planner、Manager、Worker 的流程约束、上下文边界、审批门槛、依赖判定、日志格式、交接机制和验证标准。它不决定输出语言。

这样做会让 release zip 比压缩版更大，属于正常现象。APM 的“性能”主要来自这些高密度流程约束，而不是文件体积越小越好。后续维护时不要为了减小 zip 体积删除原版契约层，也不要让英文契约层覆盖中文输出要求。
