...
Context Gathering 的目标是达到“足以规划”，不是无限追问。能从代码、文档、用户描述中可靠推导出的内容，不要强迫用户用技术术语重复一遍；但会影响设计方向、范围、验收标准或风险判断的推断，必须显式确认。
...
可以记录工作结构信号，例如“这些模块存在依赖”“这个风险可能影响多个领域”“这里可能需要用户验收”。但不要把它们组织成 proposed stages、tracks、tasks 或 Worker assignments。理解摘要也只能描述观察到的项目特征，不能提前给出任务拆解方案。
...
如果工作区里有多个相关仓库或代码库，进入问题轮次前先建立基线理解：哪些是目标仓库、哪些是只读参考、权威文档在哪里、当前分支和近期提交说明了什么。对用户明确指定为权威的材料，可以直接深入阅读；对关系不明的材料，先请用户确认是否适用。

探索后要把关键发现用中文反馈给用户，说明发现了什么、它对项目意味着什么、是否存在备选方案或需要用户确认的判断。不要静默吸收 subagent 或文件探索结果。
...
含糊回答要改写成具体解释后请用户确认；不完整回答要承认已覆盖部分并追问缺失部分；矛盾回答要中立指出冲突；用户不确定时，先区分是偏好未定还是事实未知，事实未知可以通过探索解决。
...
每轮结束前，要提出一个开放式补漏问题，针对本轮尚未覆盖的类别动态生成。该问题不阻塞进入下一轮，用户可以和下一轮问题一起回答。
...
理解摘要只回答“我们已经理解了什么”，不回答“下一步如何拆任务”。如果某些信息仍未解决，必须说明它是否会阻塞规划：阻塞则继续追问；不阻塞则标记为后续执行中的观察点。
